...
for (roots, dirs, files) in walkdir(pwd())
    for file in files
        ...
        for row in CSV.File(file, header = false)
            ...
            @test juliaE[1] ≈ factorial(s.p)/pochhammer(abs(s.m)+1,s.p)*E₀*Nₚₘ*EX
            @test juliaE[2] ≈ factorial(s.p)/pochhammer(abs(s.m)+1,s.p)*E₀*Nₚₘ*EY
            @test juliaE[3] ≈ factorial(s.p)/pochhammer(abs(s.m)+1,s.p)*E₀*Nₚₘ*EZ
            
            @test juliaB[1] ≈ factorial(s.p)/pochhammer(abs(s.m)+1,s.p)*E₀*Nₚₘ*BX
            @test juliaB[2] ≈ factorial(s.p)/pochhammer(abs(s.m)+1,s.p)*E₀*Nₚₘ*BY
            @test juliaB[3] ≈ factorial(s.p)/pochhammer(abs(s.m)+1,s.p)*E₀*Nₚₘ*BZ
        end
    end
end
